...
public static class EntityTypeUtilities
{
    ...
    public static Parameter ToParameter(AccessNode leaf, MeadowConfiguration configuration, IDbTypeNameMapper typeNameMapper)
    {
    ...
    public static ProcessedType Process(Type type, MeadowConfiguration configuration, IDbTypeNameMapper typeNameMapper)
    {
        ...
        var fullTreeMap = new FullTreeMap(type, configuration.DatabaseFieldNameDelimiter, configuration.TableNameProvider);

        WalkThroughLeaves(type, leaf =>
        {
            var parameter = ToParameter(leaf, configuration, typeNameMapper);
        ...
        if (foundAsEventStream)
        {
            ...
            var p = foundAsEventStream.Value;

            process.EventStreamType = typeof(ObjectEntry<,>).MakeGenericType(p.EventIdType, p.StreamIdType);

            process.EventStreamParameters = new List<Parameter>();

            WalkThroughLeaves(process.EventStreamType, leaf =>
                process.EventStreamParameters.Add(ToParameter(leaf, configuration, typeNameMapper)));
        }
