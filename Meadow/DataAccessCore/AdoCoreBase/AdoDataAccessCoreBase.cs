...
namespace Meadow.DataAccessCore.AdoCoreBase
{
    public abstract class AdoDataAccessCoreBase : MeadowDataAccessCoreBase<IDbCommand, IDataReader>
    {
        ...
        {
            var parameters = GetTypeParameters<TModel>();
        ...
        private Dictionary<string, FieldType> GetTypeParameters<TModel>()
        {
            var mapper = GetDbTypeNameMapper();

            var leaves = ObjectStructure.CreateStructure<TModel>(false)
                .GetDirectLeaves();

            var parameters = new Dictionary<string, FieldType>();

            leaves.ForEach(l => parameters.Add(l.Name, new FieldType
            {
                Type = l.Type,
                DbTypeName = mapper[l.Type],
        ...
        private void WriteIntoCommand(DataPoint dataPoint, IDbCommand command)
        {
            ...
            parameter.ParameterName = dataPoint.Identifier;
        ...
        protected abstract string GetSqlForCreatingTable(string tableName,
            Dictionary<string, FieldType> parameters);
