...
namespace Meadow.SqlServer.Scaffolding.Macros
{
    ...
    public class EventSourcingSqlServerMacro : MacroBase
    {
        ...
        private readonly IDbTypeNameMapper _typeNameMap = new SqlDbTypeNameMapper();
        ...
        public override string GenerateCode(params string[] arguments)
        {
            ...
            var dbGenerated = eventIdType == "bigint" ? " IDENTITY (1,1)" : "";
...
-- SPLIT
-- ---------------------------------------------------------------------------------------------------------------------
CREATE PROCEDURE {INSERT_EVENT_PROCEDURE_NAME}(@StreamId {ST_TYPE},
                                   @TypeName varchar({TYPE_SIZE}),
                                   @SerializedValue varchar({VALUE_SIZE})) 
BEGIN
